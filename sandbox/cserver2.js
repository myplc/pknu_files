...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && parsedUrl.pathname === '/') {
    ...
    <body>
      ...
      <h4>규칙4. 투표결과가 마음에 안들경우. 딱 한번 포기를 선언하면 랜덤으로 배정되며 맨 뒤로 배정 된다. 어떤 불이익도 감수해야 한다.</h4>   
      </fieldset>
      <form method="POST" action="/submit">
        ...
        <label>1지망:
          <select name="choice1" class="choice" required>
            <option>1조(5인)</option><option>2조(6인)</option><option>3조(5인)</option><option>4조(5인)</option><option selected>랜덤(하늘에 맡긴다)</option>
          </select>
        </label>
        <label>2지망:
      ...
      <script>
        ...
        selects.forEach(select => {
          select.addEventListener('change', () => {
            ...
            selects.forEach(s => {
              [...s.options].forEach(o => {
                o.disabled = values.includes(o.value) && s.value !== o.value;
              });
            });
          });
        });
        ...
        async function drawChart() {
          ...
          const colors = {
            1조(5인): '#f66', 2조(6인): '#6cf', 3조(5조): '#6f6', 4조(5조): '#fc6', '랜덤': '#aaa'
          };
    ...
    const counts = { A: 0, B: 0, C: 0, D: 0, '랜덤': 0 };
